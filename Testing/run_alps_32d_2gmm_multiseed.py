...
def _parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Train ALPS on the 32d 2-GMM with covariance scales [1, 3] and weights [0.3, 0.7] "
            "across multiple seeds, then evaluate the final saved checkpoints."
        )
    )
    ...
    parser.add_argument(
        "--sweep-name",
        default="32d_2gmm_covs_1_3_multiseed",
        help="Output directory name under alps_interpolation/ for the aggregate sweep summaries.",
    )
    parser.add_argument(
        "--ctds-root",
        default=None,
...
def main():
    ...
    sweep_dir = PROJECT_ROOT / "alps_interpolation" / f"{args.sweep_name}{balance_suffix}"
    ...
    for seed in args.seeds:
        run_name = f"32d_2gmm_covs_1_3_seed_{seed}{balance_suffix}"
