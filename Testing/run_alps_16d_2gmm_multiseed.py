...
def _parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Train ALPS on the 16d 2-GMM with covariance scales [1, 3] across multiple seeds, "
            "then evaluate the final saved checkpoints."
        )
    )
    ...
    parser.add_argument(
        "--sweep-name",
        default="16d_2gmm_covs_1_3_multiseed",
...
def _load_ctds_elbo_eubo(ctds_root: Path | None, run_name: str, seed: int):
    ...
    # Prefer obviously relevant metric/history files.
    prioritized = sorted(
        set(prioritized),
        key=lambda path: (
